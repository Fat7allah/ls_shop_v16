...
import frappe
from frappe.utils import flt, cint, random_string
import random
...
def install_demo_data():
	...
	try:
		...
		frappe.db.rollback()
		print(f"\n❌ Error during demo data installation: {str(e)}")
		import traceback
		traceback.print_exc()
		raise
	finally:
	...
	if not frappe.db.exists("Item Attribute", attribute_name):
		...
		attr = frappe.get_doc({
			...
			"item_attribute_values": [
				{"attribute_value": val, "abbr": abbr}
				for val, abbr in values
			]
		})
...
def configure_lifestyle_settings():
	...
	settings.shipping_rule = "Standard Shipping" if frappe.db.exists("Shipping Rule", "Standard Shipping") else None
	...
	if not settings.reason_for_return:
		...
		for reason_text in reasons:
			settings.append("reason_for_return", {
...
def create_style_variant(configurator_name, template_name, color, product_data):
	...
	existing = frappe.db.get_value(
		...
		{"configurator": configurator_name, "attribute_value": color},
		"name"
	)
...
def link_to_style_variant(template_name, color, size, variant_code):
	...
	style_variant = frappe.db.get_value(
		"Style Attribute Variant",
		{"configurator": configurator, "attribute_value": color},
		"name"
	)
...
def fix_product_routes():
	...
	savs = frappe.get_all("Style Attribute Variant", fields=["name", "route", "attribute_value", "item_style"])
	sav_count = 0
	...
	print(f"    Note: Routes are clean slugs - hooks.py adds /en/products/ prefix")
...
def create_default_footer_sections():
	...
	for section_data in default_sections:
		...
		settings.append("footer_sections", {
